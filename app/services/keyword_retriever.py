...
class KeywordMemoryRetriever(IMemoryRetriever):
    ...
    async def retrieve_tiered(
        ...
        Args:
            ...
            query: Query text (unused by keyword retriever)

        Returns:
            Mixed memories from all layers, ordered by importance and recency
        """
        try:
    ...
    async def _retrieve_layer(
        ...
        Args:
            entity_id: AI entity ID
            user_id: User ID filter (None for global)
            conversation_id: Conversation ID filter (None for cross-conversation)
            memory_type: Layer type (short_term, long_term, personality)
            limit: Maximum memories to retrieve

        Returns:
            List of memories from the layer
        """
    ...
    def _combine_layers(
        ...
        Args:
            short_term: Short-term memories
            long_term: Long-term memories
            personality: Personality memories
            total_limit: Maximum total memories

        Returns:
            Combined and limited memory list
        """
        guaranteed = []
        pool = []

...
class TestConversationService:
    ...
    def test_create_private_conversation_success(self, conversation_service, mock_repositories, sample_user):
        ...
        expected_conversation = Conversation(id=1, room_id=1, conversation_type=ConversationType.PRIVATE)

        mock_repositories['user_repo'].get_by_username.return_value = other_user
        mock_repositories['conversation_repo'].create_private_conversation.return_value = expected_conversation
        ...
        mock_repositories['user_repo'].get_by_username.assert_called_once_with("otheruser")
        mock_repositories['conversation_repo'].create_private_conversation.assert_called_once_with(
            room_id=1,
            participant_ids=[1, 2]
        )

    def test_create_conversation_user_not_in_room(self, conversation_service, sample_user):
    ...
    def test_create_private_conversation_wrong_participant_count(self, conversation_service, sample_user):
    ...
    def test_create_conversation_participant_not_found(self, conversation_service, mock_repositories, sample_user):
    ...
    def test_send_message_success(self, conversation_service, mock_repositories, sample_user, sample_conversation):
        ...
        mock_repositories['conversation_repo'].get_by_id.return_value = sample_conversation
        mock_repositories['conversation_repo'].is_participant.return_value = True
        mock_repositories['message_repo'].create_conversation_message.return_value = expected_message
        ...
        mock_repositories['conversation_repo'].is_participant.assert_called_once_with(1, 1)
        mock_repositories['message_repo'].create_conversation_message.assert_called_once_with(
            sender_id=1,
            conversation_id=1,
            content="Hello!"
        )

    def test_send_message_conversation_not_found(self, conversation_service, mock_repositories, sample_user):
    ...
    def test_send_message_not_participant(self, conversation_service, mock_repositories, sample_user,
                                          sample_conversation):
